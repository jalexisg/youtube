...
# Provide the attributes the tested module expects so `from yt_dlp import YoutubeDL`
# and `from pydub import AudioSegment` succeed during import.
setattr(sys.modules['yt_dlp'], 'YoutubeDL', MagicMock())
...
class TestYouTubeDownloader(unittest.TestCase):
    def setUp(self):
        ...
        # Create minimal dummy modules/attributes to avoid import-time failures
        sys.modules.setdefault('yt_dlp', ModuleType('yt_dlp'))
        sys.modules.setdefault('pydub', ModuleType('pydub'))
        setattr(sys.modules['yt_dlp'], 'YoutubeDL', MagicMock)
        setattr(sys.modules['pydub'], 'AudioSegment', MagicMock)
